import {getReq} from '@/api/organize-manager/user-manager'
import {getlisttreecurrentuserReq, getpagelisttreeReq} from '@/api/organize-manager/authorize-manager/menu-manager'
import {addReq} from '@/api/system-manager/error-record'
import {initNode, userLocalInfo} from "@/libs/utils"
import {errorRouter} from "@/router/routers";
...
export default {
  ...
  // 获取左侧导航菜单信息
  handleGetLeftMenuList({commit}) {
    return new Promise((resolve => {
      getlisttreecurrentuserReq().then(res => {
        let menuList = initNode(res.result.filter(o => o.name === 'bill-manage'))
  ...
  // 提交错误记录
  handleErrorRecord({state}, payload) {
    let obj = {
