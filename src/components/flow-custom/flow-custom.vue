<template>
  <Modal class-name="page-style-modal flow-style-modal" v-model="modalFlag" fullscreen :mask="false" :mask-closable="false"
    :closable="false">
    <div class="flow-custom">
      <!--  导航条  -->
      <div class="flow-custom-bar">
        <tool-bar ref="toolBar" @handleDragEnd="handleDragEnd" @setMode="setMode" @on-delete="onDelete"
          @on-download="onDownload" @on-format-row="onFormatRow"/>
      </div>
      <!--  流程内容  -->
              ...
              <Panel v-for="(item, i) in nodeNameList" :key="i" :name="item.id">
                ...
                <p slot="content">
                  <span v-for="(child, childIndex) in item.children" :key="childIndex" style="display: block;">
                    <i class="panel-content-node" :class="[item.iconClass, { added: child.isAdd }]" :title="child.label"
                      draggable @dragend="handleDragEnd($event, child, i)">
                      {{ child.label }}
                    </i>
                  </span>
                </p>
...
export default {
  ...
  methods: {
      ...
      if (item.nodeType !== "flowNode") {
        ...
        if (nodeItem)
          return this.$Msg.error(
            item.nodeType === "startNode" ? "只能由一个开始节点" : "只能由一个结束节点"
          );
        else {
          model.size = [30, 24];
        }
    ...
    updateFlowData(model) {
      ...
      this.updateMiniImgUrl()
      this.localSaveFlowData();
      this.$Message.success(`${this.$t("save")}${this.$t("successTips")}`);
    },
    ...
    modalOk() {
      ...
      if (flowNodes.length > 2 && this.isMoreChildMainFlow(edges, nodes))
        return this.$Msg.error("不允许有多个子级同为主流程");
