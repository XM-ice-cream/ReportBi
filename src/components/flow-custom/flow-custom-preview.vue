...
export default {
  ...
  methods: {
    ...
    init() {
      ...
      let findNode = this.graph.find('node', node => node.get('model').label === this.options.rowObj.nextProcessName)
      findNode && this.graph.setItemState(findNode, "click", true);
      // 创建网格
