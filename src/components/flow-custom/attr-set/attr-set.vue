		...
		<Split v-model="split" class="attr-set-height">
			<div slot="left" class="split-left-style">
				...
				<CellGroup @on-click="cellClick">
					<Cell v-for="(item, i) in cellList" :key="i" :title="item.title" :name="item.name" :selected="item.name === cellSelected" />
					<Cell title="抽检计划" name="samplingConfig" v-if="model.flowMain && targetProcessTypeIsSC" :selected="cellSelected === 'samplingConfig'" />
					<Cell title="抽检标准" name="samplingStandard" :selected="cellSelected === 'samplingStandard'" />
				</CellGroup>
			</div>
			<div slot="right" class="split-right-style">
				...
				<Card :bordered="false" dis-hover :padding="8" style="height: 420px; overflow-y: auto">
					...
					<Form v-if="cellSelected === 'nodeSet'" :label-width="120">
						<div v-show="eleType === 'node'">
							<FormItem :label="$t('name')">
								<treeselect
									v-model="model.labelId"
									:options="stationList"
									showCount
									disable-branch-nodes
									:placeholder="$t('pleaseSelect') + $t('name')"
									@select="nameSelect"
								/>
							</FormItem>
							...
							<FormItem label="选项">
								...
								<checkbox v-model="model.necessary" :disabled="!model.flowMain"> 是否为必要站点 </checkbox>
								<checkbox v-model="model.manual">是否支持手动过站</checkbox>
								<checkbox v-model="model.split">是否为分板站</checkbox>
								<checkbox v-model="holdprocess" @on-change="holdprocessChange"> 手动锁定制程 </checkbox>
								...
								<checkbox v-model="model.allowMulitWorkOrder">允许多个工单过站</checkbox>
							</FormItem>
							...
							<FormItem label="复测类型">
								<Input
									v-model="model.repassDefectType"
									@on-keyup="(e) => (model.repassDefectType = e.target.value.replace(/[^A-Z,]/g, ''))"
									placeholder="例如:ABB,则要在设备A测一次,设备B测试2次才进FA"
								></Input>
							</FormItem>
							<FormItem label="首件数量">
					...
					<Form v-if="cellSelected === 'equipmentBind' && eleType === 'node'" :label-width="120">
						...
						<FormItem v-if="!isPreview">
							<Button type="primary" @click="eqpBind">{{ `${$t("equipmentBind")}${$t("submit")}` }} </Button>
						</FormItem>
					</Form>
					<!--   QTime设置   -->
					<AttrSetQTime
						v-if="cellSelected === 'qTime' && eleType === 'node'"
						:model="model"
						:optList="optList"
					...
					<AttrSetTrackGo
						ref="refTrackGo"
						v-show="cellSelected === 'trackGo' && eleType === 'node'"
						:model="model"
						:isShow="!isPreview"
					...
					<!--   抽检计划   -->
					<SamplingConfig v-if="cellSelected === 'samplingConfig' && eleType === 'node'" :model="model" :optList="optList" :isShow="!isPreview" />
					<!--   抽检标准   -->
					<SamplingStandard v-if="cellSelected === 'samplingStandard' && eleType === 'node'" :model="model" :optList="optList" :isShow="!isPreview" />
					<!--   生成条码   -->
					<CreateBarCode
						ref="createBarCode"
						v-show="cellSelected === 'createBarCode' && eleType === 'node'"
						:model="model"
						:isShow="!isPreview"
						@on-createBarCode-submit="createBarCodeSubmit"
					/>
				</Card>
			</div>
		</Split>
...
export default {
	...
	data() {
		return {
			...
			renderRow(row) {
				let result = "";
				if (row && Object.keys(row).length) result = row.remark ? `<b>${row.enCode}</b> - [${row.remark}]` : `<b>${row.enCode}</b>`;
				return result;
			},
		};
	},
	...
	methods: {
		...
		trackChange(obj) {
			...
			if (this.trackGoState) this.$Msg.error(`${this.$t("trackIn")}、${this.$t("trackOut")}${this.$t("noEmpty")}`);
		...
		nodeAttrChange() {
			if (this.eleType === "node") {
				if (this.trackGoState) return this.$Msg.error(`${this.$t("trackIn")}、${this.$t("trackOut")}${this.$t("noEmpty")}`);
				let color = "";
		...
		//获取锁定制程状态
		getProcessState() {
			getProcessStateReq({ routeId: this.optList.id, processId: this.model.labelId }).then((res) => {
				if (res.code === 200) {
