...
<template>
	<Form ref="submitReq" :model="submitData" :rules="ruleValidate" :label-width="120" :label-colon="true">
		...
		<!-- 抽检比例 -->
		<FormItem :label="$t('globalScale')" prop="globalScale" v-show="submitData.samplingType === 'globalScale'">
			<InputNumber style="width: unset" v-model="submitData.globalScale" :min="1" :max="100" :precision="0"></InputNumber>
			%
		</FormItem>
		<!-- 间隔时间 -->
		<FormItem :label="$t('interval')" prop="intervalTime" v-show="submitData.samplingType === 'interval'">
			每
			<InputNumber style="width: unset" v-model="submitData.intervalTime" :min="1" :precision="0"></InputNumber>
			分钟抽
			<InputNumber style="width: unset" v-model="submitData.intervalAmount" :min="1" :precision="0"></InputNumber>
			个
		</FormItem>
		<!-- 固定比例 -->
		<FormItem :label="$t('fixedScale')" prop="fixedBase" v-show="submitData.samplingType === 'fixedScale'">
			每
			<InputNumber style="width: unset" v-model="submitData.fixedBase" :min="1" :precision="0"></InputNumber>
			个抽
			<InputNumber style="width: unset" v-model="submitData.fixedScale" :min="1" :max="submitData.fixedBase" :precision="0"></InputNumber>
			个
		</FormItem>
		<!-- 计划抽检生效时间 -->
	...
	methods: {
		...
		submit() {
			this.$refs.submitReq.validate((validate) => {
				if (validate) {
					let { samplingType, globalScale, intervalTime, intervalAmount, fixedBase, fixedScale } = this.submitData;
