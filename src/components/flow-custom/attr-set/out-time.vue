<template>
	<Form ref="submitReq" :model="submitData" :rules="ruleValidate" :label-width="90">
				...
				<FormItem :label="$t('stationOutName')" prop="stationOutName">
					...
					>
						<Option v-for="(item, i) in stationOutNameList" :value="item.id" :key="i">{{
							item.id != 0
								? item.fromProcessName + "(" + item.fromProcessRuleName + ")---" + item.name + "(" + item.toProcessRuleName + ")"
								: item.name
						}}</Option>
					</Select>
				</FormItem>
		...
		<div class="station-box">
			<Icon class="delete" type="md-close-circle" @click="deleteStation" v-if="stationOutName !== 0" />
			<Row>
				<i-col span="12">
					<!-- 行为 -->
				...
				<i-col span="12">
					...
					<FormItem :label="`${$t('fromProcess')}Rule`" prop="fromProcessRuleName">
						<Select v-model="submitData.fromProcessRuleName" :placeholder="$t('pleaseSelect') + $t('fromProcessRuleName')">
							<Option v-for="(item, i) in ruleNameData" :value="item.detailCode" :key="i">{{ item.detailName }}</Option>
						</Select>
					</FormItem>
				</i-col>
				<!-- 结束流程 -->
				...
				<i-col span="12">
					<FormItem :label="`${$t('toProcess')}Rule`" prop="toProcessRuleName">
						<Select v-model="submitData.toProcessRuleName" :placeholder="$t('pleaseSelect') + $t('toProcessRuleName')">
							<Option v-for="(item, i) in ruleNameData" :value="item.detailCode" :key="i">{{ item.detailName }}</Option>
						</Select>
					</FormItem>
				</i-col>
		...
		<div v-if="isShow" style="text-align: center">
			<Button type="primary" @click="add" style="margin-right: 10px" v-if="stationOutName === 0 && addFlag === false">{{ $t("Add") }}</Button>
			<Button type="primary" @click="submit" v-if="stationOutName !== 0 || addFlag === true">{{ $t("submit") }}</Button>
		</div>
	</Form>
</template>
...
export default {
	...
	methods: {
		...
		deleteStation() {
			...
			this.$Msg.success(`${this.$t("delete") + this.$t("successTips")}`);
