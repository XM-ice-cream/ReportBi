  ...
  <div class="upload-custom">
    <Upload :show-upload-list="false"
      ...
      <slot name="camera" v-if="uploadIcon === 'ios-camera'">
        <div
            style="text-align: center;border: 1px dashed #ccc;border-radius: 4px;"
            :style="{
            width: `${uploadHeight}px`,
...
export default {
  ...
  methods: {
    ...
    handleSuccess(response, file, fileList) {
      this.$Spin.hide();
      if (response.code === 200) this.$emit("upload-success", file, fileList);
      else this.$Msg.error(`${this.$t("fail")},${response.message}`);
    },
    /**
     * 上传失败触发
     */
