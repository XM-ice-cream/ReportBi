<template>
  <div class="avatar-custom">
      <Avatar
              v-if="imgBase64.includes('data:image/')"
              :size="avatarSize"
              :src="imgBase64"
              @click.native="visible = !visible"
      />
      <Avatar
              v-else-if="imgBase64.includes('iconfont')"
              :size="avatarSize"
              :custom-icon="imgBase64"
      />
...
export default {
  ...
  methods: {
    getImbBase64() {
      ...
      getheadiconReq({ fileName: this.imgUrl })
        .then(async (res) => {
          this.imgBase64 = `data:image/png;base64,${btoa(
