...
import {userLocalInfo} from "./utils"
...
const errorInfo = ({loading}, {config, data, status, statusText}) => {
  ...
  if (!ignoreUrlList.includes(config.url)) {
    ...
    // 提交错误记录
    let {baseURL, url, method, data, params, headers: {Authorization}} = config
    const payload = {
      baseUrl: baseURL, url, method, data, params, Authorization, status, errorCode: status, errorCodeMsg: null,
...
export function requestCenter(params) {
  ...
  instance.interceptors.response.use(res => {
    // 响应成功
    if (params.loading) Spin.hide()
    const {status, config: {baseURL, url, method, headers: {Authorization}}} = res
