  ...
  <div class="page-style oee-charts">
    ...
    <Drawer v-model="drawerFlag" :title="drawerTitle" width="520" :mask-closable="false" @on-close="cancelClick">
      <Form ref="req" :model="defaultReq" :rules="ruleValidate" :label-width="80" :label-colon="true">
        ...
        <!-- 按钮 -->
        <FormItem>
          <drawer-button @on-cancel="cancelClick" @on-ok="submitClick" @on-update="submitClick" @on-okAndClose="submitClick(true)"></drawer-button>
        </FormItem>
      </Form>
    </Drawer>
    <!-- 页面表格 -->
        ...
        <i-col span="6" v-for="(item, index) in data" :key="index">
          <div class="card_item">
            <Card>
              ...
              <OeePie style="height:150px" :data="item" :ref="'oeePie' + index" :index="String(index)" />
            </Card>
          </div>
        </i-col>
      </Row>
