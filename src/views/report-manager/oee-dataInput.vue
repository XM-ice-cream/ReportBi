<template>
  <div class="page-style ">
    ...
    <Drawer v-model="drawerFlag" :title="drawerTitle" width="520" :mask-closable="false" @on-close="cancelClick">
      <Form ref="req" :model="defaultReq" :rules="ruleValidate" :label-width="80" :label-colon="true">
        <FormItem :label="$t('timeHorizon')" prop="dateTime">
          <DatePicker v-model="defaultReq.dateTime" type="datetimerange" format="yyyy/MM/dd HH:mm:ss" :placeholder="$t('pleaseSelect') + $t('timeHorizon')" style="width: 100%" @on-change="datePickerChange"></DatePicker>
        </FormItem>
        ...
        <FormItem :label="defaultReq.type === 'Line' ? $t('mfgLine') : $t('station')" v-if="defaultReq.type === 'Line' || defaultReq.type === 'Station'">
          <Select v-model="defaultReq.value" multiple :max-tag-count="2" filterable :placeholder="
              defaultReq.type === 'Line'
                ? $t('pleaseSelect') + $t('mfgLine')
                : $t('pleaseSelect') + $t('station')
            ">
            <Option v-for="(item, index) in lineOrStationList" :value="item" :key="index">{{ item }}
            </Option>
          </Select>
        </FormItem>
      ...
      <div class="setting_con">
        ...
        <div v-if="req.dateTime.length == 2" class="searchData">
          ...
          <div v-if="req.type === 'ALL'">
            <Input search enter-button width="200" v-model="defaultReq.equipmentName" @on-search="submitClick" :placeholder="$t('pleaseEnter') + $t('equipmentName')" />
          </div>
        </div>
      </div>
      ...
      if (this.req.dateTime.length == 2) {
        let params = {
          ...
          data: {
            ...
            type:
              !!this.req.equipmentName.trim() && this.req.type === "ALL"
                ? "BYEQPNAME"
                : this.req.type,
            value:
              this.req.value.join(",") ||
              this.lineOrStationList.join(",") ||
              this.req.equipmentName.trim(),
          },
        };
        this.data = [];
        this.req.total = 0;
