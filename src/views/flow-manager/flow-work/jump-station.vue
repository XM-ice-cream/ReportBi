	...
	methods: {
		...
		submitClick() {
			this.$refs.submitReq.validate((validate) => {
				if (validate) {
					if (!this.submitData.processId) return this.$Msg.error(`${this.$t("pleaseEnter")}${this.$t("jumpWhere")}`);
					...
					skipReq(obj).then((res) => {
						...
						this.errData.unshift({
							...
							text: `${this.$t("barCoding")}:${obj.barCodes[0]},${this.$t("workOrder")}:${obj.workOrder},${this.$t("currentSite")}:${
								obj.curProcessName
							},${this.$t("nextSite")}:${obj.nextProcessName},${this.$t("jumpWhere")}:${obj.processName}${errText}`,
						});
					});
				}
			});
			...
			getlistReq(obj).then(async (res) => {
				if (res.code === 200) {
					...
					if (this.flowList.length) {
						...
						const { nodes, edges } = flowItem.json ? JSON.parse(flowItem.json) : { nodes: [], edges: [] };
						const node = nodes.map((o) => {
							...
							return {
								...
								trackOutMethods: hasChild(o, "trackOutMethods") ? o.trackOutMethods : ["SaveDefectInfoAsync"], // 出站
