				...
				<div slot="title">
					<Row>
								...
								<div class="poptip-style-content" slot="content">
									<Form ref="searchReq" :model="req" :label-width="80" :label-colon="true" @submit.native.prevent>
										...
										<FormItem :label="$t('process')" prop="processId" v-show="req.workOrder">
											<Select v-model="req.processId" transfer filterable clearable :placeholder="$t('pleaseSelect') + $t('process')" label-in-value>
										...
										<FormItem :label="$t('line')" prop="lineName">
											<Select v-model="req.lineName" transfer filterable clearable :placeholder="$t('pleaseSelect') + $t('line')">
										...
										<FormItem :label="$t('bigBarcode')" prop="panelNo">
											<Input v-model.trim="req.panelNo" :placeholder="$t('pleaseEnter') + $t('bigBarcode')" @keyup.native.enter="searchClick" />
										</FormItem>
										<!-- 小条码 -->
										<FormItem :label="$t('sn')" prop="unitId">
											<Input v-model.trim="req.unitId" :placeholder="$t('pleaseEnter') + $t('sn')" @keyup.native.enter="searchClick" />
										</FormItem>
										<!-- 56条码 -->
										<FormItem :label="$t('unitId56')" prop="unitId56">
											<Input v-model="req.unitId56" :placeholder="$t('pleaseEnter') + $t('unitId56')" @keyup.native.enter="searchClick" />
										</FormItem>
										<!-- 56条码 -->
										<FormItem :label="$t('boxCode')" prop="boxNo">
											<Input v-model="req.boxNo" :placeholder="$t('pleaseEnter') + $t('boxCode')" @keyup.native.enter="searchClick" />
										</FormItem>
										<!-- 56条码 -->
										<FormItem :label="$t('cartonCode')" prop="cartonNo">
											<Input v-model="req.cartonNo" :placeholder="$t('pleaseEnter') + $t('cartonCode')" @keyup.native.enter="searchClick" />
										</FormItem>
										<!-- 56条码 -->
										<FormItem :label="$t('palletCode')" prop="palletNo">
											<Input v-model="req.palletNo" :placeholder="$t('pleaseEnter') + $t('palletCode')" @keyup.native.enter="searchClick" />
										</FormItem>
										<!-- 56条码 -->
										<FormItem :label="$t('carrier')" prop="carrier">
											<Input v-model="req.carrier" :placeholder="$t('pleaseEnter') + $t('carrier')" @keyup.native.enter="searchClick" />
										</FormItem>
						...
						<i-col span="12">
							<button-custom
								:btnData="btnData"
								@on-jumpStation-click="jumpStationClick"
								@on-unlock-click="unlockClick"
...
export default {
	...
	methods: {
		...
		pageLoad() {
			...
			let { inProcessTime, outProcessTime, workOrder, lineName, panelNo, unitId, unitId56, processId, boxNo, cartonNo, palletNo, carrier, enabled } =
				this.req;
		...
		exportClick() {
			...
			if (
				...
				return this.$Msg.warning("查询条件不允许为空");
			let obj = {
				inProcessTime: inProcessTime ? formatDate(inProcessTime) : "",
				outProcessTime: outProcessTime ? formatDate(outProcessTime) : "",
				workOrder,
