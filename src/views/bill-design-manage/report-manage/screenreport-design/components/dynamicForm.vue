		...
		<Form :label-width="80" label-position="left">
			<template v-for="(item, index) in options">
				<div v-if="isShowForm(item, '[object Object]')" :key="index">
					<FormItem
						v-if="inputShow[item.name] && item.type != 'dycustComponents' && item.type != 'dynamic-add-table'"
						:label="item.label"
						:prop="item.name"
					...
					>
						<InputNumber
							...
							@on-change="changed($event, item.name)"
						/>
						...
						<Select
							...
							clearable
							placeholder="请选择"
							@on-change="(val) => changed(val, item.name)"
						>
							...
							<p slot="content">
								<template v-for="(itemChildList, idx) in itemChild.list">
									<FormItem :key="idx" :label="itemChildList.label" :prop="itemChildList.name" :required="itemChildList.required">
										<InputNumber
											v-if="itemChildList.type == 'InputNumber'"
											size="small"
											style="width: 100%"
											v-model="formData[itemChildList.name]"
											controls-position="right"
											:placeholder="itemChildList.placeholder"
											@on-change="changed($event, itemChildList.name)"
										/>

										<Input
											v-if="itemChildList.type == 'Input'"
										...
										<Input
											v-if="itemChildList.type == 'Input-textarea'"
											v-model.trim="formData[itemChildList.name]"
											size="small"
											type="textarea"
											:rows="2"
										...
										<i-switch
											v-if="itemChildList.type == 'i-switch'"
											v-model="formData[itemChildList.name]"
											placeholder="请输入内容"
											size="small"
											@on-change="changed($event, itemChildList.name)"
										...
										<Upload
											v-if="itemChildList.type == 'Upload-picture'"
											size="small"
										...
										<RadioGroup
											v-if="itemChildList.type == 'RadioGroup'"
											v-model="formData[itemChildList.name]"
											@on-change="(val) => changed(val, itemChildList.name)"
										>
											<Radio v-for="it in itemChildList.selectOptions" :key="it.code" :label="it.code">{{ it.name }}</Radio>
										</RadioGroup>

										<Select
											v-if="itemChildList.type == 'Select'"
											size="small"
											v-model="formData[itemChildList.name]"
											clearable
											placeholder="请选择"
										...
										<Slider
											v-if="itemChildList.type == 'Slider'"
											v-model="formData[itemChildList.name]"
											@on-change="(val) => changed(val, itemChildList.name)"
										/>
									</FormItem>
