		...
		<Form :label-width="80" label-position="left" class="dynamicComponent-search">
			...
			<FormItem v-for="(item, index) in userNameList" :key="index" :label="item.paramName">
				...
				<Input
					...
					v-else-if="item.paramType === 'Array'"
					clearable
					size="small"
				/>
				<DatePicker
					v-else
					v-model="item.sampleItem"
					transfer
					type="datetime"
			...
			<FormItem label="行">
				<div class="row">
					<draggable group="site" v-model="rowParamList" class="site-flex" chosenClass="item">
						<Dropdown
							v-for="(item, index) in rowParamList"
							:key="`${item}${index}`"
							transfer
							trigger="click"
							@on-click="(name) => dropDownClick(name, index, 'row')"
						>
							<div class="item">
			...
			<FormItem label="列">
				<div class="columns">
					<draggable group="site" v-model="columnsParamList" class="site-flex" chosenClass="item">
						<Dropdown
							v-for="(item, index) in columnsParamList"
							:key="`${item}${index}`"
							transfer
							trigger="click"
							@on-click="(name) => dropDownClick(name, index, 'column')"
						>
							<div class="item">
								<Icon type="ios-arrow-down"></Icon>
								{{ item }}
