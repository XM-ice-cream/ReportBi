			...
			<Form ref="submitReq" :model="rightForm" :label-width="110" :label-colon="true" @submit.native.prevent>
				...
				<template v-if="rightForm.userDefinedType === 'condition'">
					...
					<Table
						...
						disabled-hover
						draggable
						@on-drag-drop="changeRow"
						v-if="rightForm.type === 'ordinary'"
					>
						<!-- 可选列 -->
					...
					>
						...
						<template slot-scope="{ index }" slot="defined">
							<Button type="primary" class="row-button" @click="addRowClick(index)"> 定义 </Button>
						</template>
						<!-- 关系 -->
						<template slot-scope="{ index }" slot="relation">
					...
					<Timeline class="timeline">
						...
						<TimelineItem v-for="(item, index) in connectDate" :key="index">
							...
							<p class="time">
								{{
									item.selectItem
										? item.relation + " (" + item.selectItem + ") " + item.operator + "" + item.content
										: item.relation + " " + item.formula
								}}
							</p>
						</TimelineItem>
					</Timeline>
...
export default {
	...
	data() {
		return {
			...
			tableData: [{ selectItem: "apple", operator: "=", type: "string", content: "", relation: "add" }],
			tableData2: [{ formula: "sum(123)", relation: "add" }],
			selectList: {
				//可选列集合
				selectItemList: [
