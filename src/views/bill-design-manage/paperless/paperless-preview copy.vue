	...
	methods: {
		...
		getTable() {
			...
			celldata.forEach((item, index) => {
				...
				// td 内部输入框、复选框、时间选择器
				let divInner = "";
				let defaultValue = "a"; //默认值
				if (cellType) {
					if (cellType.type === "input") {
						divInner = "<Input type='text' v-model=this.value />";
						console.log(divInner);
					}
					if (cellType.type === "checkbox") {
						defaultValue = [cellType.default];
						divInner = ` <CheckboxGroup value="['ES']"" >`;
						cellType.data.forEach((item) => {
							divInner += ` <Checkbox label="${item.value}">${item.name}</Checkbox>`;
						});
						divInner += `</CheckboxGroup>`;
						console.log(divInner);
					}
				}

				this.tableHtml[r][c] = { style, colspan, rowspan, divStyle, value: v, divInner, defaultValue: defaultValue, r, c };
			});
		},
		checkClick(e, r, c) {
			console.log(e, e.inputType, e.target.value, r, c);

			this.resultData[r] = this.resultData[r] || [];
			this.resultData[r][c] = this.resultData[r][c] || [];
			const value = e.target.value;
			const index = this.resultData[r][c].indexOf(value);

			if (!e.inputType) {
				//复选框
				if (!this.resultData[r][c].includes(value)) this.resultData[r][c].push(value);
				else this.resultData[r][c].splice(index, 1);

				this.resultData[r][c] = [...this.resultData[r][c]];
			} else {
				//输入框
				this.resultData[r][c] = value;
			}
