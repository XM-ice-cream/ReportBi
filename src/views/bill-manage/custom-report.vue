									...
									>
										...
										<FormItem :label="$t('customSQL')" prop="sql">
											<v-selectpage
												...
												key-field="id"
												show-field="name"
												:data="customSQLPageListUrl"
												v-model="req.sql"
												@values="sqlOnChange"
												:placeholder="$t('pleaseSelect') + $t('customSQL')"
												:result-format="
													(res) => {
														return {
															totalRow: res.total,
										...
										<FormItem
											:label="item.title"
											:prop="'parameters.' + [item.parameterName]"
											:rules="{
												required: true,
												type: item.type === 'number' ? 'number' : item.type === 'datetime' ? 'date' : 'string',
												message: $t('pleaseEnter') + item.title,
												trigger: 'change',
											}"
											v-for="(item, index) in parameterJson"
											:key="index"
										>
											<Input
												v-if="item.type === 'array'"
												v-model="req.parameters[item.parameterName]"
												type="textarea"
												:rows="3"
												:placeholder="'每行一个' + item.title"
											/>
...
<script>
import { executepagequeryReq, exportReq, customSQLPageListUrl } from "@/api/system-manager/custom-export-sql";
