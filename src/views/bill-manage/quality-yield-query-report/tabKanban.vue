		...
		<Card :bordered="false" dis-hover class="card-style">
			...
			<div class="wip-table">
				...
				<Table
					...
					:highlight-cell="tableConfig.highlightRow"
					:height="tableConfig.height"
					:loading="Yieldloading"
					:columns="columnsYield"
					:data="dataYield"
					@on-cell-click="yieldClick"
					:span-method="handleSpanYield"
				>
				</Table>
			</div>
			<div class="wip-table">
				...
				<Table
					...
					:columns="columnsQty"
					:data="dataQty"
					@on-cell-click="qtyClick"
					:span-method="handleSpanQty"
				>
				</Table>
			</div>
		</Card>
	...
	methods: {
		...
		pageLoad() {
			...
			if (startTime && endTime) {
				...
				this.$Msg.warning("请输入查询条件!");
			}
		},
		//获取LPA良率
		async getLpaYeild(obj) {
			await getLpaYieldReq(obj)
				.then((res) => {
					...
					if (res.code === 200) {
						...
						if (this.dataYield) {
							//站点与段别的对应关系
							const title = [
								{ section: "Overall", yieldtype: "Overall" },
								{ section: "SMT", yieldtype: "Input" },
								{ section: "SMT", yieldtype: "Output" },
								{ section: "SMT", yieldtype: "Defect" },
								{ section: "SMT", yieldtype: "Fyp" },
								{ section: "SMT", yieldtype: "After_Rework" },
								{ section: "ENCAPE", yieldtype: "Input" },
								{ section: "ENCAPE", yieldtype: "Output" },
								{ section: "ENCAPE", yieldtype: "Defect" },
								{ section: "ENCAPE", yieldtype: "Rate" },
								{ section: "BE", yieldtype: "Input" },
								{ section: "BE", yieldtype: "Output" },
								{ section: "BE", yieldtype: "Defect" },
								{ section: "BE", yieldtype: "Rate" },
							];
							//创建空的表格信息
		...
		async getLpaDefectYeild(obj) {
			await getLpaDefeactYieldReq(obj)
				.then((res) => {
					this.tableConfig.loading = false;
					if (res.code === 200) {
						let result = [];
						if (this.dataYield) {
							//站点与段别的对应关系
							const title = [
								{ section: "SMT", station: "Op40" },
								{ section: "SMT", station: "AutoOnoff" },
								{ section: "ENCAPE", station: "Op45Op50" },
								{ section: "ENCAPE", station: "Autoonoff2" },
								{ section: "ENCAPE", station: "Autoonoff3" },
								{ section: "ENCAPE", station: "Op60" },
								{ section: "ENCAPE", station: "Op70" },
								{ section: "BACK END", station: "Function" },
								{ section: "BACK END", station: "I16" },
								{ section: "BACK END", station: "Fvi" },
							];
							//创建空的表格信息
							result = title.map((item) => {
