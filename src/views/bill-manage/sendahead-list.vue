		...
		<Drawer v-model="drawerFlag" :title="`Sendahead List${isAdd ? '新增' : '编辑'}`" width="650" :mask-closable="false" :before-close="cancelClick">
			<Form ref="submitReq" :model="submitData" :label-width="80" :label-colon="true" @submit.native.prevent>
				...
				<FormItem label="机种" prop="project">
					<Input v-model="submitData.project" :placeholder="$t('pleaseEnter') + '机种'" />
				</FormItem>
				<!-- 工厂 -->
				<FormItem label="工厂" prop="factory">
					<Input v-model="submitData.factory" :placeholder="$t('pleaseEnter') + '工厂'" />
				</FormItem>
				<!-- 仓位 -->
				<FormItem label="仓位" prop="wareHouse">
					<Input v-model="submitData.wareHouse" :placeholder="$t('pleaseEnter') + '仓位'" />
				</FormItem>
				<!-- 脚位 -->
				...
				<FormItem label="料号" prop="ipn">
					<Input v-model="submitData.ipn" :placeholder="$t('pleaseEnter') + '料号'" />
				</FormItem>
				<!-- APN -->
			...
			<Card :bordered="false" dis-hover class="card-style">
							...
							<Poptip v-model="poptipModal" class="poptip-style" placement="right-start" width="350" trigger="manual">
								...
								<div class="poptip-style-content" slot="content">
									<Form ref="searchReq" :model="req" :label-width="80" :label-colon="true" @submit.native.prevent>
										...
										<!-- 机种 -->
										<FormItem label="机种" prop="project">
											<Input v-model="req.project" :placeholder="$t('pleaseEnter') + '机种'" />
										</FormItem>
										<!-- 工厂 -->
										<FormItem label="工厂" prop="factory">
											<Input v-model="req.factory" :placeholder="$t('pleaseEnter') + '工厂'" />
										</FormItem>
										<!-- 仓位 -->
										<FormItem label="仓位" prop="wareHouse">
											<Input v-model="req.wareHouse" :placeholder="$t('pleaseEnter') + '仓位'" />
										</FormItem>
										<!-- 脚位 -->
										<FormItem label="脚位" prop="location">
				...
				>
					...
					<template slot-scope="{ row }" slot="operator">
						<div style="cursor: pointer; color: blue" @click="getStepList(row)">setup</div>
					</template>
				</Table>
				<page-custom
		...
		<Modal v-model="modalFlag" title="SetupStation" width="800" :mask="false" :mask-closable="false" @on-cancel="modalFlag = false">
			<Table :columns="setupColumns" :data="setupData" height="500px">
				<template slot-scope="{ row }" slot="checkOut">
					<Checkbox v-model="row.checkOut"></Checkbox>
				</template>
			</Table>
			<template #footer>
	...
	data() {
		return {
			...
			columns: [
				...
				{ title: this.$t("operator"), slot: "operator", width: 125, align: "center" },
			], // 表格数据
			data: [{}], // 表格数据
			setupData: [{}],
			setupColumns: [
				...
				{ title: "检查状态", key: "checkState", align: "center", tooltip: true },
