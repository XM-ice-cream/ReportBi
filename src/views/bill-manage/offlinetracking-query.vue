								...
								<div class="poptip-style-content" slot="content">
									<Form ref="searchReq" :model="req" :label-width="60" :label-colon="true" @submit.native.prevent>
										...
										<FormItem label="PanelNo" prop="panelNo">
											<Input
												v-model="req.panelNo"
												:placeholder="$t('pleaseEnter') + 'PanelNo' + $t('multiple,separated')"
...
export default {
	...
	methods: {
		...
		pageLoad() {
			...
			const { workOrder, lineName, startTime, endTime, sn, panelNo, eqpId, type } = this.req;
			let obj = {
		...
		// 导出
		exportClick() {
			const { workOrder, lineName, startTime, endTime, panelNo, sn, eqpId, type } = this.req;
			let obj = {
				workOrder, // 工单
				lineName: lineName.toString(), // 线体名称
				panelNo,
