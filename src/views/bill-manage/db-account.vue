			...
			<Form ref="submitData" :model="submitData" :rules="ruleValidate" :label-width="120" :label-colon="true" @submit.native.prevent>
				...
				<FormItem label="员工工号" prop="userID">
					<v-selectpage
						...
						key-field="userID"
						show-field="userID"
						:data="userIDPageListUrl"
						v-model="submitData.userID"
						:disabled="!isAdd"
						:placeholder="$t('pleaseSelect') + '员工工号'"
						:params="{
							enabled: '',
						}"
						:result-format="
...
export default {
	...
	data() {
		return {
			...
			ruleValidate: {
				accountID: [{ required: true, message: "账户ID必填", trigger: "blur" }],
				userID: [{ required: true, message: "员工工号必填", trigger: "blur" }],
				area: [{ required: true, message: "区域必填", trigger: "blur" }],
				dbName: [{ required: true, message: "DB名称必填", trigger: "blur" }],
			},
		};
	},
	activated() {
	...
	methods: {
		...
		editClick() {
			if (this.selectObj) {
				let { userID, accountID, accountType, privilegeLevel, area, dbName, status, remark, enabled, id } = this.selectObj;
				this.submitData = {
					userID,
					accountID,
					accountType,
