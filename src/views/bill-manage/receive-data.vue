                ...
                <div class="poptip-style-content" slot="content">
                  ...
                  <Form ref="searchReq" :model="req" :label-width="70" :label-colon="true">
                    ...
                    <FormItem :label="$t('startTime')" prop="startTime">
                      <DatePicker transfer type="datetime" :placeholder="$t('pleaseSelect') + $t('startTime')" format="yyyy-MM-dd HH:mm:ss" :options="$config.datetimeOptions" v-model="req.startTime" style="width: 280px"></DatePicker>
                    </FormItem>
                    <!-- 结束时间 -->
                    <FormItem :label="$t('endTime')" prop="endTime">
                      <DatePicker transfer type="datetime" :placeholder="$t('pleaseSelect') + $t('endTime')" format="yyyy-MM-dd HH:mm:ss" :options="$config.datetimeOptions" v-model="req.endTime" style="width: 280px"></DatePicker>
                    </FormItem>
                    <!-- 工单 -->
                    ...
                    <FormItem :label="$t('panelNo')" prop="panelNo">
                      <Input type="textarea" :autosize="{minRows: 1,maxRows: 5}" v-model.trim="req.panelNo" :placeholder="$t('pleaseEnter') + $t('panelNo')+'(请以逗号或回车分隔)'" />
                    </FormItem>
                    <!-- dataKey -->
                    <FormItem :label="$t('dataKey')" prop="dataKey">
                      <Input type="textarea" :autosize="{minRows: 1,maxRows: 5}" v-model.trim="req.dataKey" :placeholder="$t('pleaseEnter') + 'dataKey'+'(请以逗号或回车分隔)'" />
                    </FormItem>
                    <!-- dataType -->
                    <FormItem :label="$t('dataType')" prop="dataType">
                      <Input type="textarea" :autosize="{minRows: 1,maxRows: 5}" v-model.trim="req.dataType" :placeholder="$t('pleaseEnter') + $t('dataType')+'(请以逗号或回车分隔)'" />
                    </FormItem>
                    <!-- 设备编码 -->
                    <FormItem :label="$t('eqpCode')" prop="eqpCode">
                      <Input type="textarea" :autosize="{minRows: 1,maxRows: 5}" v-model.trim="req.eqpCode" :placeholder="$t('pleaseEnter') + $t('eqpCode')+'(请以逗号或回车分隔)'" />
                    </FormItem>
