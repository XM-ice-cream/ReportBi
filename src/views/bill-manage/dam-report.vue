            ...
            <div class="poptip-style-content" slot="content">
              <Form class="poptip-style-content" ref="searchReq" :model="req" :label-width="80" :label-colon="true" @submit.native.prevent>
                ...
                <FormItem :label="$t('process')" prop="stepName">
                  <v-selectpage class="select-page-style" ref="stepName" v-if="poptipModal" key-field="name" show-field="name" :data="processPageListUrl" v-model="req.stepName" :placeholder="$t('pleaseSelect') + $t('process')" :result-format="
                        (res) => {
                        return {
                            totalRow: res.total,
                            list: res.data || [],
                ...
                <FormItem :label="$t('pn')" prop="pn">
                  <v-selectpage ref='pn' class="select-page-style" v-if="poptipModal" key-field="id" show-field="pn" :data="pnPageListUrl" v-model="req.pn" :placeholder="$t('pleaseSelect') + $t('pn')" :result-format="
                  (res) => {
                    return {
                      totalRow: res.total,
                      list: res.data || [],
                ...
                <FormItem :label="$t('equipment')" prop="eqpId">
                  <v-selectpage class="select-page-style" ref='eqpId' v-if="poptipModal" key-field="enCode" show-field="enCode" :data="eqpPageListUrl" v-model="req.eqpId" :placeholder="$t('pleaseSelect') + $t('equipment')" :result-format="
                        (res) => {
                            return {
                            totalRow: res.total,
                            list: res.data || [],
                ...
                <FormItem :label="$t('workOrder')" prop="workOrder">
                  <v-selectpage ref="workOrder" class="select-page-style" v-if="poptipModal" key-field="workOrder" show-field="workOrder" :data="workerPageListUrl" v-model="req.workOrder" :placeholder="$t('pleaseSelect') + $t('workOrder')" :result-format="
                    (res) => {
                        return {
                        totalRow: res.total,
                        list: res.data || [],
    ...
    pageLoad () {
      ...
      let params = {
        ...
        data: {
          ...
          lineName, pn, eqpId, workOrder, stepName
        }
