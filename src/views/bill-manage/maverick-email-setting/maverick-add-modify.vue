      ...
      <Form ref="submitData" :model="submitData" :label-width="90" :label-colon="true" @submit.native.prevent>
       <!-- 群组类型 -->
        ...
        <FormItem label="邮箱群组" prop="emaiL_GROUP">
            <Input v-model.trim="submitData.emaiL_GROUP" type="textarea" :autosize="{minRows: 2,maxRows: 5}" :placeholder="$t('pleaseEnter') + '邮箱群组'"  clearable/>
        </FormItem>
         <!-- 目标值 -->
...
export default {
  ...
  data () {
    return {
        groupTypeList:['FACA','EarlyWarning'],//群组类型
      submitData:{
        grouP_TYPE: "",
        grouP_NAME: "",
        grouP_LINE: "",
  ...
  methods: {
    ...
    submitClick (isClose = false) {      
      this.$refs.submitData.validate((validate) => {
        if (validate) {
          const { grouP_TYPE,grouP_NAME,grouP_LINE,grouP_STATION,grouP_MODEL,grouP_INFO,emaiL_GROUP,grouP_TARGET,grouP_GOAL,id} = this.submitData;
          const obj = {
             grouP_TYPE,grouP_NAME,grouP_LINE,grouP_STATION,grouP_MODEL,grouP_INFO,
             emaiL_GROUP:commaSplitString(emaiL_GROUP).join(";"),
             grouP_TARGET,grouP_GOAL,id
          }
