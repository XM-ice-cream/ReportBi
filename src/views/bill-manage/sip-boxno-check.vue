      ...
      <Card :bordered="false" dis-hover class="card-style" id="sip-boxno-check">
        <Form ref="submitReq" :model="req" :label-width="80" :label-colon="false" style="width: 80%;margin:0 atuo" @submit.native.prevent>
          <Row>
            ...
            <Col :xs="24" :sm="24" :md="12" :lg="8">
            <FormItem label='内箱号' prop="boxNo">
              <label>{{req.boxNo}}</label>
            </FormItem>
            </Col>
            <Col :xs="24" :sm="24" :md="12" :lg="8">
...
export default {
  ...
  methods: {
      ...
      checkBoxNoReq(obj).then(res => {
        if (res.code === 200) {
          if (!res.result) {
            ...
            this.req = { ...this.req, pn: "", apn: null, cartonNo: "", inputCartonNo: "", boxNo: "", inputBoxNo: "", shipAddr: "", cartonQTY: "", };
            return;
          };
    ...
    //核验内箱号
    checkBoxNo () {
      const { boxNo, inputBoxNo, pn } = this.req;
      const checkInput = inputBoxNo.split("$");

      if (boxNo === checkInput[0] && pn === checkInput[3]) {
        //输入框聚焦
        inputSelectContent(this.$refs.inputCartonNo);
        this.tipMsg = "";
      } else {
        this.tipMsg = "NG 检查失败 与内箱条码或者料号不匹配!";
      }
    },
    // 核验外箱码
    checkCartonNo () {
      const { cartonNo, inputCartonNo, pn } = this.req;
      const checkInput = inputCartonNo.split("$");
      if (cartonNo === checkInput[0] && pn === checkInput[3]) {
        this.tipMsg = "OK 检查成功";
      } else {
        this.tipMsg = "NG 检查失败 与外箱条码或者料号不匹配!";
      }
    },
    // 重置
    reset () {
