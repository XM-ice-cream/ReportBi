...
<script>
import { getpagelistReq, exportReq } from "@/api/bill-manage/productiondata-byTime-report";
import { getButtonBoolean, formatDate, exportFile, treeLoop } from "@/libs/tools";
...
export default {
	name: "productiondata-byTime-report",
	data() {
		return {
			poptipModal: false,
			...
			columns: [
				...
				{ title: this.$t("workOrder"), key: "workorder", align: "center", minWidth: 150 },
				{ title: this.$t("stepName"), key: "stepname", align: "center", minWidth: 120, tooltip: true },
				{ title: this.$t("lineName"), key: "linename", align: "center", minWidth: 140, tooltip: true },
				{ title: this.$t("input_PassQty"), key: "inputqty", align: "center", minWidth: 140, tooltip: true },
				{ title: this.$t("output_PassQty"), key: "outputqty", align: "center", minWidth: 120, tooltip: true },
				{ title: this.$t("input_DefectQty"), key: "defectqty", align: "center", minWidth: 100, tooltip: true },
			], // 表格数据
		};
	},
	activated() {
	...
	methods: {
		...
		pageLoad() {
			...
			let { startTime, endTime, workOrder, stepName } = this.req;
			if ((startTime && endTime) || workOrder || stepName) {
		...
		exportClick() {
			...
			exportReq(obj).then((res) => {
				...
				const fileName = `${this.$t("productiondata-byTime-report")}${formatDate(new Date())}.xlsx`; // 自定义文件名
