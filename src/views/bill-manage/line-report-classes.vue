                ...
                <div class="poptip-style-content" slot="content">
                  <Form ref="searchReq" :model="req" :label-width="60" :label-colon="true" @submit.native.prevent>
                    ...
                    <FormItem :label="$t('time')" prop="dateTime">
                      <DatePicker transfer type="date" :placeholder="$t('pleaseSelect') + $t('time')" format="yyyy-MM-dd" :options="$config.datetimeOptions" v-model="req.dateTime"></DatePicker>
                    </FormItem>
                    <!-- 线体 -->
...
export default {
  ...
  data () {
    return {
      ...
      columns: [
        {
          title: this.$t("sort"), type: "index", width: 50, align: "center", fixed: "left",
          indexMethod: (row) => {
  ...
  methods: {
    ...
    pageLoad () {
      ...
      getpagelistReq(obj).then((res) => {
        ...
        if (res.code === 200) {
          ...
          const pageData = data || [];
          this.data = pageData.map(item => { return { ...item, total: this.getTotal(item) } })
    ...
    exportClick () {
      ...
      exportReq(obj).then((res) => {
        ...
        const fileName = `${this.$t('line-report-classes')}_${obj.data.lineName}_${obj.data.dateTime}.xlsx`; // 自定义文件名
