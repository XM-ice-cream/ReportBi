                ...
                <div class="poptip-style-content" slot="content">
                  <Form ref="searchReq" :model="req" :label-width="80" :label-colon="true" @submit.native.prevent @keyup.native.enter="searchClick">
                    ...
                    <FormItem label="MES站点" prop="stepName">
                        <Input v-model.trim="req.stepName" :placeholder="$t('pleaseEnter') + 'MES站点'"  />
                    </FormItem>
                     <!-- 客户站点 -->
                     <FormItem label="客户站点" prop="customerStepName">
                        <Input v-model.trim="req.customerStepName" :placeholder="$t('pleaseEnter') + '客户站点'"  />
                    </FormItem>
                     <!-- 上传站点 -->
                     <FormItem label="上传站点" prop="uploadStepName">
                        <Input v-model.trim="req.uploadStepName" :placeholder="$t('pleaseEnter') + '上传站点'"  />
                    </FormItem>
                    <!-- 序号 -->
                    <FormItem label="序号" prop="sortNumber">
                        <InputNumber v-model.trim="req.sortNumber" :placeholder="$t('pleaseEnter') + '序号'"  />
                    </FormItem>
                  </Form>
...
<script>
import { getpagelistReq ,attendanceInsightUploadUrl } from "@/api/bill-manage/insight-tracktooling.js";
import {  getButtonBoolean,formatDate, renderDate } from "@/libs/tools";
import AddModify from "./insight-tracktooling/add-modify.vue";
import UploadCustom from "@/components/upload-custom";


export default {
  name: "insight-trakctooling",
  ...
  methods: {
    ...
    pageLoad () {
        this.data = [];
        this.tableConfig.loading = false;
        const { startTime, endTime,modelName,customerModelName,stepName,customerStepName,uploadStepName,sortNumber} = this.req;
        this.$refs.searchReq.validate((validate) => {
            ...
            let obj = {
                ...
                data: {
                    ...
                    modelName,customerModelName,stepName,customerStepName,uploadStepName,sortNumber
                },
            };
