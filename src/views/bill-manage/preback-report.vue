                ...
                <div class="poptip-style-content" slot="content">
                  <Form ref="searchReq" :model="req" :label-width="80" :label-colon="true" @submit.native.prevent @keyup.native.enter="searchClick" :rules="ruleValidate">
                    ...
                    <FormItem :label="$t('inProcessTime')" prop="inProcessTime">
                      <DatePicker transfer type="datetime" :placeholder="$t('pleaseSelect') + $t('inProcessTime')" format="yyyy-MM-dd HH:mm:ss" :options="$config.datetimeOptions" v-model="req.inProcessTime"></DatePicker>
                    </FormItem>
                    <!-- 离开制程时间 -->
                    <FormItem :label="$t('outProcessTime')" prop="outProcessTime">
                      <DatePicker transfer type="datetime" :placeholder="$t('pleaseSelect') + $t('outProcessTime')" format="yyyy-MM-dd HH:mm:ss" :options="$config.datetimeOptions" v-model="req.outProcessTime"></DatePicker>
                    </FormItem>
                    <!-- 线体名称 -->
                    <FormItem :label="$t('line')" prop="lineName">
                      <v-selectpage class="select-page-style" v-if="searchPoptipModal" key-field="name" show-field="name" :data="linePageListUrl" v-model="req.lineName" :placeholder="$t('pleaseEnter') + $t('line')" :result-format="
                          (res) => {
                            return {
                              totalRow: res.total,
                              list: res.data || [],
