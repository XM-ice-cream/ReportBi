...
export default {
		...
		return {
			...
			ruleValidate: {
				userID: [
					...
					{ validator: this.checkUserID, trigger: "blur" },
				],
				userName: [{ required: true, message: "员工姓名必填", trigger: "blur" }],
			},
		};
	},
	activated() {
	...
	methods: {
		...
		editClick() {
			if (this.selectObj) {
				let { userID, userName, deptName, remark, enabled, id } = this.selectObj;
				this.submitData = {
					userID,
					userName,
					deptName,
