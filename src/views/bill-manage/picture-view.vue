      ...
      <Card :bordered="false" dis-hover class="card-style">
        ...
        <Table :border="tableConfig.border" :highlight-row="tableConfig.highlightRow" :height="tableConfig.height" :loading="tableConfig.loading" :columns="columns" :data="data">
          <template slot-scope="{ row }" slot="image">
            <Button style="height:16px" v-if="row.remotePath" type="primary" size="small" @click="previewImage(row.remotePath,row.directoryType)">{{ $t("preview") }}</Button>
            <Button style="height:16px;margin-left:5px" v-if="row.remotePath" type="primary" size="small" @click="downloadImage(row.remotePath,row.directoryType,row.fileName)">{{ $t("download") }}
            </Button>
          </template>
        </Table>
