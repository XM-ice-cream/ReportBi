/* EncapFillScrap明细数据 */
<template>
  <Modal draggable v-model="modalFlag" width="1250" title="EncapFillScrap明细" :styles="{ top: '20px' }" :closable="false">
...
export default {
  ...
  watch: {
    isShow () {
      if (this.isShow) {
        ...
        const { lineName, stepName } = this.paramData
        this.pageLoad(lineName, stepName)
      }
    }
  },
  activated () {
  ...
  methods: {
    // 获取分页列表数据
    pageLoad (lineName, stepName) {
      console.log('pageLoad1', lineName);
      console.log('pageLoad2', stepName);
      this.data = [];
      let obj = {
    ...
    pageChange (index) {
      ...
      this.pageLoad(this.paramData.lineName,this.paramData.stepName)
    },
    // 选择一页有条数据
