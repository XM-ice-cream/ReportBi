...
<script>
import { addReq,modifyReq } from "@/api/bill-manage/insight-consumable";
export default {
  name: "insight-consumable-add-modify",
  props:{
    drawerFlag:{
        type:Boolean,
        default:false
  ...
  methods: {
    ...
    submitClick (isClose = false) {      
      this.$refs.submitData.validate((validate) => {
        if (validate) {
          const { modelName,customerModelName,stepName,uploadStepName,id} = this.submitData;
          const obj = {
            modelName,customerModelName,stepName,uploadStepName,id
          }
