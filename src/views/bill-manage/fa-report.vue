      ...
      <Card :bordered="false" dis-hover class="card-style">
                ...
                <div class="poptip-style-content" slot="content">
                  ...
                  <Form ref="searchReq" :model="req" :label-width="80" :label-colon="true">
                    ...
                    <FormItem :label="$t('line')" prop="lineName">
                      <Select v-model="req.lineName" clearable transfer filterable label-in-value multiple @on-select="lineSelect" :placeholder="`${$t('pleaseSelect')}${$t('line')}`">
                    ...
                    <FormItem :label="$t('model')" prop="modelName">
                      <v-selectpage class="select-page-style" ref="modelName" v-if="poptipModal" key-field="modelName" show-field="modelName" :data="modelPageListUrl" v-model="req.modelName" :placeholder="$t('pleaseSelect') + $t('model')" :result-format="
                          (res) => {
                            return {
                              totalRow: res.total,
                              list: res.data || [],
                    ...
                    <FormItem :label="$t('panelNo')" prop="panelNo">
                      <Input type="textarea" :autosize="{ minRows: 1, maxRows: 5 }" v-model.trim="req.panelNo" @keyup.native.enter="searchClick" :placeholder="$t('pleaseEnter') + $t('panelNo')" />
                    </FormItem>
                    <!-- 不良原因 -->
                    <FormItem :label="$t('badCause')" prop="failureReason">
                      <v-selectpage class="select-page-style" v-if="poptipModal" multiple key-field="name" show-field="name" :data="defectReasonPageListUrl" v-model=" req.failureReason" :placeholder="$t('pleaseSelect') + $t('badCause')" :result-format="
                          (res) => {
                            return {
                              totalRow: res.total,
                              list: res.data || [],
                    ...
                    <FormItem :label="$t('badPhenomenon')" prop="errorCode">
                      <v-selectpage class="select-page-style" v-if="poptipModal" multiple key-field="name" show-field="name" :data="defectdescPageListUrl" v-model="req.errorCode" :placeholder="$t('pleaseSelect') + $t('badPhenomenon')" :result-format="
                          (res) => {
                            return {
                              totalRow: res.total,
                              list: res.data || [],
                    ...
                    <FormItem :label="$t('machineNumber')" prop="epqId">
                      <v-selectpage class="select-page-style" ref="eqpId" v-if="poptipModal" key-field="enCode" show-field="enCode" :params="{ category: 'Auto' }" :data="eqpPageListUrl" v-model="req.epqId" :placeholder="$t('pleaseSelect') + $t('machineNumber')" :result-format="
                          (res) => {
                            return {
                              totalRow: res.total,
                              list: res.data || [],
                    ...
                    <FormItem :label="$t('repair') + $t('eqpCode')" prop="reworkMachineId">
                      <Input v-model.trim="req.reworkMachineId" :placeholder="$t('pleaseEnter') + $t('repair') + $t('eqpCode')" @keyup.native.enter="searchClick" />
                    </FormItem>
                    <!-- 料号 -->
                    <FormItem :label="$t('pn')" prop="pn">
                      <v-selectpage class="select-page-style" ref="pn" v-if="poptipModal" key-field="pn" show-field="pn" :data="pnPageListUrl" v-model="req.pn" :placeholder="$t('pleaseSelect') + $t('pn')" @values="pnSelectChange" :result-format="
                          (res) => {
                            return {
                              totalRow: res.total,
                              list: res.data || [],
                    ...
                    <FormItem :label="$t('workOrder')" prop="wo">
                      <v-selectpage multiple ref="workOrder" v-if="poptipModal" class="select-page-style" key-field="workOrder" show-field="workOrder" :data="workerPageListUrl" v-model="req.wo" :placeholder="$t('pleaseSelect') + $t('workOrder')" :result-format="
                          (res) => {
                            return {
                              totalRow: res.total,
                              list: res.data || [],
                    ...
                    <FormItem :label="$t('process')" prop="failureStationName">
                      <v-selectpage class="select-page-style" v-if="poptipModal" ref="failureStationName" multiple key-field="name" show-field="name" :data="processPageListUrl" v-model="req.failureStationName" :placeholder="$t('pleaseSelect') + $t('process')" :result-format="
                          (res) => {
                            return {
                              totalRow: res.total,
                              list: res.data || [],
        ...
        <Table :border="tableConfig.border" :highlight-row="tableConfig.highlightRow" :height="tableConfig.height" :loading="tableConfig.loading" :columns="columns" :data="data" :span-method="handleSpan">
          ...
          <template slot-scope="{ row }" slot="op20Image">
            <img class="img_item" @click="imgClick('data:image/bmp;base64,'+item)" :src="'data:image/bmp;base64,'+item" v-for="(item, index) in row.op20Image && row.op20Image.split(',')" :key="index" />
          </template>
          <template slot-scope="{ row }" slot="op25Image">
            <img class="img_item" @click="imgClick('data:image/bmp;base64,'+item)" :src="'data:image/bmp;base64,'+item" v-for="(item, index) in row.op25Image && row.op25Image.split(',')" :key="index" />
          </template>
          <template slot-scope="{ row }" slot="op30Image">
            <img class="img_item" @click="imgClick('data:image/bmp;base64,' + item)" :src="'data:image/bmp;base64,' + item" v-for="(item, index) in row.op30Image && row.op30Image.split(',')" :key="index" />
          </template>
