   ...
   <!-- 右侧抽屉 Form表单 -->
    <Drawer v-model="drawerFlag" :title="drawerTitle" width="500" :mask-closable="false" @on-close="cancelClick">
      <Form ref="submitData" :model="submitData" :label-width="90" :label-colon="true" @submit.native.prevent>
        ...
        <!-- IC脚位 -->
        <FormItem label="IC脚位" prop="refdes">
            <Input v-model.trim="submitData.refdes" :placeholder="$t('pleaseEnter') + 'IC脚位'"  />
        </FormItem>
        <!-- 按钮 -->
        <FormItem>
  ...
  methods: {
    ...
    submitClick (isClose = false) {      
      this.$refs.submitData.validate((validate) => {
        if (validate) {
          const { modelName,customerModelName,refdes,id} = this.submitData;
          const obj = {
            modelName,customerModelName,refdes,id
          }
